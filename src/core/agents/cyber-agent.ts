...
export async function getAnswerByThreatIntel(inputData: DataRequest, context: string, provider: LLMProvider) { 
    const { question }: DataRequest = inputData;

    const agent = await getAgent(inputData, provider, context, [new CybersecurityAPITool()]);
